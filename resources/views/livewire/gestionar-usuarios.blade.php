        ...
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12">
                <div class="card">
                                    ...
                                    <tbody>
                                        @forelse ($usuarios as $usuario)
                                            <tr class="text-center">
                                                ...
                                                <td style="vertical-align:middle;">{{ $usuario->ap_paterno." ".$usuario->ap_materno." ".$usuario->name }}</td>
                                                <td style="vertical-align:middle;">{{ $usuario->f_tipo_documento_id."-".$usuario->nro_documento }}</td>
                                                <td style="vertical-align:middle;">{{ $usuario->email }}</td>
                                                <td style="vertical-align:middle;"> @foreach ($usuario->roles as $role) {{$role->name}} @endforeach</td>
                                                <td style="vertical-align:middle;">
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input" type="checkbox" id="campo-obligatio-1" @if ($usuario->locked == 1) checked @endif wire:target="cambiar_estado_suspension({{$usuario->id}})" wire:loading.attr="disabled" wire:click="cambiar_estado_suspension({{$usuario->id}})">
                                                    </div>
                                                </td>
                                                <td style="vertical-align:middle;" >
                    ...
                    @include('administrador.usuarios.parts.usuario-modal')
                </div>
            </div>
        </div>
    </div>
